 ...
 *
 * @param numbers - 一個數字陣列
 * @param _predicate - 一個函式，用來判斷元素是否符合條件
 * @returns - 回傳一個數字陣列，表示過濾後的結果
 */
export function filterArray(numbers: number[], _predicate: (num: number) => boolean): number[] {
    // 請在此處寫下你的程式碼
        return numbers.filter(_predicate);
}
 ...
 *
 * @param _numbers - 一個數字陣列
 * @param _transform - 一個函式，用來轉換元素
 * @returns - 回傳一個數字陣列，表示轉換後的結果
 */
export function transformArray(_numbers: number[], _transform: (num: number) => number): number[] {
    // 請在此處寫下你的程式碼
    return _numbers.map(_transform);
}
